...
void FRPolyconeForBrep::GenerateBrepData(FRBrepGenerator* factory_ptr) const
{
	...
	const FRVector2* vertex_table = segment.GetTable();	// segment table
	const int N = segment.GetSize();		// segment number of division 
	...
	for(j = 0; j < fNum; j++)
	{
		for(i = 0; i < N; i++)
		{
			...
			XX = vertex_table[i].x();			//	normal vector.x()
			YY = vertex_table[i].y();			//	normal vector.y()
	...
	const int planeN = fNum - 1;			// number of plane : z(n-1) -> z(n)
...
} // FRPolyconeForBrep::GenerateBrepData()
