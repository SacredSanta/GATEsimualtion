...
//	void		operator()(size_t n)		{ resize(n); }
	...
	FRArray<T>& operator=(const FRArray<T>& X) { return assign(X); }
...
FRArray<T>::FRArray()
	{ init(0); }
template <class T>
FRArray<T>::FRArray(size_t n)
	{ init(0); mAlloc(n); }
template <class T>
FRArray<T>::FRArray(const T& src, size_t n)
	{ init(0); assign(src, n); }
template <class T>
FRArray<T>::FRArray(const T* src, size_t n)
	{ init(0); assign(src, n); }
template <class T>
FRArray<T>::FRArray(const FRArray<T>& X)
	{ init(0); assign(X); }
template <class T>
FRArray<T>::~FRArray()
	{ init(1); }
...
template <class T>
FRArray<T>& FRArray<T>::assign(const T& t, size_t n)
{
	size_t i;
	T* dst;
...
template <class T>
FRArray<T>& FRArray<T>::assign(const T *src, size_t n)
{
	size_t i;
	T* dst;
...
template <class T>
FRArray<T>& FRArray<T>::assign(const FRArray<T>& X)
{
	if(this != &X)
	{	mAlloc(X.mSize);
...
template <class T>
void FRArray<T>::init(FRBoolean constructed)
{
...
template <class T>
void FRArray<T>::mAlloc(size_t n, FRBoolean saved)
{
	if(n == 0)
		init(1);
	else if(n == mSize)
